...
describe("dSTAKE Ecosystem - Yield Accrual and Exchange Rate Update", function () {
  ...
  it("should accrue yield over time, improve exchange rate, and allow correct withdrawals", async function () {
    ...
    const dUsdCollaterals = config.dStables.dUSD.collaterals.filter(
      (addr) => addr !== ethers.ZeroAddress
    );
    const collateralAsset = dUsdCollaterals[dUsdCollaterals.length - 1];
